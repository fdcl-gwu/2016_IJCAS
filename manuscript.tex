...
\begin{enumerate}[(i)]
    ...
    \item \label{item:prop_psi_quadratic} \( \Psi \) is a locally quadratic function, which means there exists constants \( 0 < n_1 \leq n_2 \) such that
    \begin{align}\label{eq:psi_bound}
        n_1 \norm{e_R}^2 \leq \Psi(R) \leq n_2 \norm{e_R}^2 ,
    \end{align}
    on the domain
    \begin{align}\label{eq:psi_quadratic_domain}
        D_1 = \braces{R \in \SO \vert e_{R} = 0 \iff R = R_d } .
    \end{align}
...
Consider a domain \( D_2 \) about the desired attitude defined as
\begin{align}
	D_2 = \braces{R \in \SO \vert \Psi < \psi < h_1, r^T R^T v < \beta < \cos \theta}. \label{eqn:domain}
\end{align}
...
\begin{gather}
	0 < c < \min \braces{\sqrt{\frac{2 k_R n_1}{\lambda_m}},\sqrt{\frac{2 k_R n_2}{\lambda_M}}, \frac{4 k_R k_\Omega}{k_\Omega^2 + 4 k_R \lambda_M H}} , \label{eqn:c_bound}
\end{gather}
...
We choose the domain \( D_2 \) to exclude the undesired critical points of \( A(R) \) as well as the infeasible regions defined by the state inequality constraints. 
Furthermore, one can show that the set \( D_2 \) is an open neighborhood of the desired attitude configuration, \( R_d\).
...
Furthermore, the domain \( D_3 \) is an open neighborhood of the desired attitude. 

...
\end{align*}
over the domain \( D_3 = D_1 \cup D_2\), which ensures that the properties of~\Cref{prop:config_error,prop:error_dyn} are satisfied.

From~\Cref{prop:config_error}, the configuration error function is locally quadratic and it is bounded in \( D_3 \) by
\begin{align*}
    n_1 \norm{e_R} < \Psi <  n_2 \norm{e_{R}}^2 ,
\end{align*}
for constants \( 0 < n_1 \leq n_2 \)~\cite[Definition 6.28]{bullo2004}.

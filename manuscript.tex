...
%	\item \label{item:prop_crit}The error function \( \Psi \) is minimized when \(R = R_d\).
    ...
    \end{align}
    on the domain \( D \) about the desired attitude \( R_d \)
    \begin{align}\label{eq:psi_quadratic_domain}
        D = \braces{R  \vert \Psi < \psi < h_1, r^T R^T v < \beta < \cos \theta} .
    \end{align}
\end{enumerate}
\end{prop}
\begin{proof}
...
Consider the domain \( D \), given in~\cref{prop:config_error}, about the desired attitude, then the following statements hold:
\begin{enumerate}[(i)]
...
More explicitly, the set \( D \) of the metric space \( \parenth{R \in \SO, \Psi(R,R_d)} \) is an open neighborhood of \( R_d \) if there exists an \( \epsilon > 0 \) such that for any \( R \in \SO | \Psi(R,R_d) < \epsilon \), it is also true that \( R \in D \).
...
The single equilibrium point occurs at \( R=R_d\) and results in \( e_R = 0 \) and \( \Psi = A = 0\).
...
From~\Cref{prop:config_error}, the configuration error function is locally quadratic and it is bounded in \( D \) by~\cref{eq:psi_bound}~\cite[Definition 6.28]{bullo2004}.

...
This behavior is characterized by situations where the rigid body starts close to the desired attitude, yet the system unnecessarily rotates through a large angle inspite of a small initial error. 
...
\begin{figure}[H]%
    \centering 
    \subfigure[Attractive \( A(R) \) \label{fig:attract_error}]{\includegraphics[trim={8mm 0 8mm 0}, clip]{figures/attract_error.pdf}}~
    \subfigure[Repulsive \( B(R) \) \label{fig:avoid_error} ]{\includegraphics[trim={8mm 0 8mm 0}, clip]{figures/avoid_error.pdf}}~
    \subfigure[Combined \( \Psi \) \label{fig:combined_error}]{\includegraphics[trim={8mm 0 8mm 0}, clip]{figures/combined_error.pdf}}%
...
\begin{figure}[b]
    \centering 
    \subfigure[Attitude error vector \(e_R\) components \label{fig:eR_con}]{\input{pgf/eR_noadapt.pgf}}~
    \subfigure[Configuration error \( \Psi \) \label{fig:Psi_con}]{\input{pgf/Psi_noadapt.pgf}}~
    \subfigure[Angle to each constraint \label{fig:con_angles_con}]{\input{pgf/ang_con_noadapt.pgf}} 
...
\begin{figure} 
  \centering 
  \subfigure[Configuration error \( \Psi \)\label{fig:Psi_adapt}]{\input{pgf/Psi_adapt.pgf}}~
  \subfigure[Angle to each constraint \label{fig:con_angles}]{\input{pgf/ang_con_adapt.pgf}}\\
  \subfigure[Disturbance estimate \( \bar \Delta \) components \label{fig:delta_adapt} ]{\input{pgf/dist_adapt.pgf}}~
...
\begin{figure}[H]
    \centering 
    \subfigure[{Configuration error \( \Psi \)} \label{fig:Psi_tv} ]{\input{pgf/Psi_timevarying.pgf}}~
    \subfigure[{Angle to constraint } \label{fig:con_angle_tv}]{\input{pgf/ang_con_timevarying.pgf}}~
    \subfigure[{Disturbance estimate \(\bar\Delta\) components} \label{fig:Delta_tv}]{\input{pgf/dist_timevarying.pgf}}
    \caption{Time-varying external disturbance simulation}
    \label{fig:tv} 
\end{figure}
\begin{figure}[t]
    \centering 
    \subfigure[{Attitude error vector \(e_R\)} components\label{fig:eR_exp}]{\input{pgf/eR_exp.pgf}}~
    \subfigure[{Configuration error \( \Psi \)} \label{fig:Psi_exp} ]{\input{pgf/Psi_exp.pgf}}~
    \subfigure[{Control input \( u\) } components\label{fig:u_exp}]{\input{pgf/u_exp.pgf}}\\
    \subfigure[{Attitude Trajectory } \label{fig:traj_exp}]{\includegraphics[width=0.3\columnwidth]{traj_exp} }~
    \subfigure[{Angle to constraint } \label{fig:con_angle_exp}]{\input{pgf/ang_con_exp.pgf}}

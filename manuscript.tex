...
\begin{figure}[H]%
    \centering 
    \subfigure[Attractive \( A(R) \) \label{fig:attract_error}]{\includegraphics[width=0.3\columnwidth]{attract_error} }~
    \subfigure[Repulsive \( B(R) \) \label{fig:avoid_error} ]{\includegraphics[width=0.3\columnwidth]{avoid_error} }~
    \subfigure[Combined \( \Psi \) \label{fig:combined_error}]{\includegraphics[width=0.3\columnwidth]{combined_error} }%
...
\subsection{Attitude Parameterizations}
...
We model the external disturbance as the superposition of constant and time-varying terms as
\begin{align*}
...
\Cref{fig:tv} demonstrates the ability for the adaptive controller, presented in~\cref{prop:adaptive_control}, to handle time-varying disturbances.
We can see in~\cref{fig:Delta_tv} that that estimate \(\bar \Delta \) accurately tracks the true disturbance after approximately~\SI{5}{\second}.
In addition,~\cref{fig:con_angle_tv} shows that the constraint is never verified as the angle between the body-fixed sensor \( r \) and the constraint \( v \) is greater than \SI{12}{\degree} over the entire attitude maneuver.
\end{multicols}
\begin{figure}[H]
    \centering 
...
Furthermore, we can show that the set \( D \) is an open neighborhood of the desired attitude configuration, \( R_d\).

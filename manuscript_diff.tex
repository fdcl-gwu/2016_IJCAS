...
%DIF DEL manuscript.tex       Mon Jan 16 09:11:00 2017
%DIF ADD manuscript_new.tex   Fri Jan 13 15:54:02 2017
%DIF < \documentclass[fleqn]{IJCAS}  % Comment this line out if you need a4paper
%DIF -------
\documentclass[10pt,fleqn]{IJCAS}  % Comment this line out if you need a4paper %DIF > 
%DIF -------
\usepackage{ijcas_packages}
%DIF 3a3
\allowdisplaybreaks[1] %DIF > 
%DIF -------
...
\begin{keywords}
\DIFaddbegin \DIFadd{adaptive control, }\DIFaddend attitude control, \DIFdelbegin \DIFdel{asymptotic stability, }\DIFdelend \DIFaddbegin \DIFadd{constraint, obstacle, }\DIFaddend special orthogonal group
\DIFdelbegin \DIFdel{, constraint
}\DIFdelend %At least four key words or phrases in alphabetical order, separated by commas.
\end{keywords}
...
\DIFaddbegin \DIFadd{We use the notation \( \hat{x}\) and \( \parenth{x}^\wedge\) interchangeably.
In particular, we use the latter form when the expression for the argument of the }\textit{\DIFadd{hat}} \DIFadd{map is long. 
}\DIFaddend Several properties of the hat map are summarized as
\begin{gather}
   %DIF <  \hat x y = x\times y = - y\times x = - \hat y x,\\
    \DIFadd{\hat x y = x\times y = - y\times x = - \hat y x, }\label{eqn:cross_product}\\
    \DIFadd{x}\DIFaddend \cdot \hat y z = y\cdot \hat z x,\quad \hat x\hat y z = (x\cdot z) y - (x\cdot y ) z\label{eqn:STP},\\
...
\squeezeup
\section{Attitude Control on $\SO$ with Inequality Constraints}\DIFaddbegin \label{sec:attitude_control}
\DIFaddend 
...
\begin{enumerate}[(i)]
	...
	\begin{align}\DIFaddbegin \label{eq:dirDiff_A}
		\DIFaddend \dirDiff{A}{R} &= \eta \cdot e_{R_A} \DIFdelbegin \DIFdel{.
	}\DIFdelend \DIFaddbegin \DIFadd{,
	}\DIFaddend \end{align}
	\DIFaddbegin \DIFadd{where the notation \( \dirDiff{A}{R} \) represents the directional derivative of $A$ with respect to $R$ along $\delta R$.
	}\DIFaddend \item \label{item:prop_erb} The variation of \( B(R) \) with respect to a variation of \( \delta R = R \hat{\eta} \) for \( \eta \in \R^3 \) is given by
	\begin{align}\DIFaddbegin \label{eq:dirDiff_B}
		\DIFaddend \dirDiff{B}{R} &= \eta \cdot e_{R_{B}}.
	\end{align}
%DIF >     where the notation \( \D_R A \) and \(\D_R B\) represents the derivative of \( A \) or \( B \) with respect to \( R\).
%	\item \label{item:prop_crit}The error function \( \Psi \) is minimized when \(R = R_d\).
	...
	\end{align*}
    \DIFaddbegin \item \label{item:prop_psi_quadratic} \DIFadd{\( \Psi \) is a locally quadratic function, which means there exist constants \( 0 < n_1 \leq n_2 \) such that
    }\begin{align}\DIFadd{\label{eq:psi_bound}
        n_1 \norm{e_R}^2 \leq \Psi(R) \leq n_2 \norm{e_R}^2 ,
    }\end{align}
    \DIFadd{on the neighborhood $D$ of the desired attitude \( R_d \)
    }\begin{align}\DIFadd{\label{eq:psi_quadratic_domain}
        D = \braces{R\in\SO  \vert \Psi < \psi < h_1, r^T R^T v < \beta < \cos \theta}
    }\end{align}
    \DIFadd{for $0<\psi < h_1 $ and $0< \beta<\cos\theta$. 
...
\DIFaddend \begin{figure}[H]%
    \centering 
    \DIFdelbeginFL %DIFDELCMD < \subfigure[Attractive \( A(R) \) \label{fig:attract_error}]{\includegraphics[width=0.45\columnwidth]{attract_error} }%%%
\DIFdelFL{~
    }%DIFDELCMD < \subfigure[Repulsive \( B(R) \) \label{fig:avoid_error} ]{\includegraphics[width=0.45\columnwidth]{avoid_error} }\\%%%
%DIF < add desired spacing between images, e. g. ~, \quad, \qquad, \hfill etc. %(or a blank line to force the subfigure onto a new line) 
    %DIFDELCMD < \subfigure[Configuration \( \Psi \) \label{fig:combined_error}]{\includegraphics[width=0.45\columnwidth]{combined_error} }%%%
\DIFdelendFL \DIFaddbeginFL \subfigure[Attractive \( A(R) \) \label{fig:attract_error}]{\includegraphics[width=0.33\columnwidth]{attract_error} }\DIFaddFL{~
    }\subfigure[Repulsive \( B(R) \) \label{fig:avoid_error} ]{\includegraphics[width=0.33\columnwidth]{avoid_error} }\DIFaddFL{~
    }\subfigure[Combined \( \Psi \) \label{fig:combined_error}]{\includegraphics[width=0.33\columnwidth]{combined_error} }\DIFaddendFL %
    \caption{\DIFaddbeginFL \DIFaddFL{Visualization of }\DIFaddendFL Configuration \DIFdelbeginFL \DIFdelFL{error function visualization}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Error Functions using spherical coordinate representation}\DIFaddendFL }
    \label{fig:config_error} 
\end{figure}%
\DIFaddbegin \begin{multicols}{2}
\DIFaddend We apply this concept and parametrize the rotation matrix \( R \in \SO \) in terms of the spherical angles \( \SI{-180}{\degree} \leq \lambda \leq \SI{180}{\degree}  \) and \( \SI{-90}{\degree} \leq \beta \leq \SI{90}{\degree} \). 
...
The \DIFaddbegin \DIFadd{horizontal axes of~}\cref{fig:config_error} \DIFadd{represent the domain of the spherical angles \( \lambda \) and \( \beta \) in degrees, while the vertical axes represent the unitless magnitude of the error functions defined in~}\cref{eqn:psi,eqn:A,eqn:B}\DIFadd{.
The }\DIFaddend attractive error function, given by~\cref{eqn:A}, has been previously used for attitude control on \(\SO\).
...
A visualization of the \DIFdelbegin \DIFdel{configuration }\DIFdelend \DIFaddbegin \DIFadd{repulsive }\DIFaddend error function is presented in~\cref{fig:avoid_error} which shows that as the boundary of the constraint is neared, or \( r^T R^T v \to \cos \theta \), the barrier term increases, \( B \to \infty\).
...
We first assume that there is no disturbance, i.e., \( \Delta = 0 \)\DIFaddbegin \DIFadd{, }\DIFaddend and present a nonlinear controller in~\Cref{prop:att_control}.
...
Consider \DIFdelbegin \DIFdel{a domain }\DIFdelend \DIFaddbegin \DIFadd{the neighborhood }\DIFaddend \( D \)\DIFaddbegin \DIFadd{, given in~}\cref{prop:config_error}\DIFadd{, }\DIFaddend about the desired attitude\DIFdelbegin \DIFdel{defined as
}\begin{eqnarray*}
	\DIFdel{D = \braces{R \in \SO \vert \Psi < \psi < h_1, r^T R^T v < \beta < \cos \theta}. %DIFDELCMD < \label{eqn:domain}%%%
}\end{eqnarray*}
%DIFAUXCMD
\DIFdel{Then }\DIFdelend \DIFaddbegin \DIFadd{, then }\DIFaddend the following statements hold:
\begin{enumerate}[(i)]
	\item \label{item:prop_eR_dot_bound_AB} Upper bounds of \( A(R) \) and \( B(R) \) are given by
	\begin{gather}
		\norm{A} < \DIFaddbegin \DIFadd{b_2 }\norm{e_{R_A}}\DIFadd{^2 < }\DIFaddend c_A  , \quad \norm{B} < c_B . \label{eqn:AB_bound}
	\end{gather}
  \DIFaddbegin \DIFadd{where the constant \( b_2\) is given by \( b_2 = \frac{h_1 h_4}{h_5 \parenth{h_1 - \psi}}\) for
  }\begin{align*}
    \DIFadd{h_4 }&\DIFadd{= \min\braces{g_1 + g_2, g_2 + g_3 , g_3 + g_1} ,}\\
    \DIFadd{h_5 }&\DIFadd{= \min\braces{\parenth{g_1 + g_2}^2,\parenth{g_2 + g_3}^2 , \parenth{g_3 + g_1}^2} .}\\
  \end{align*}
	\DIFaddend \item \label{item:prop_eR_dot_bound_EF} Upper bounds of \( E(R,R_d) \) and \( F(R) \) are given by
	\begin{gather}
...
\begin{figure}[b]
    \centering 
    \subfigure[Attitude error vector \(e_R\) components \label{fig:eR_con}]{\includegraphics[width=0.3\columnwidth]{eR_con}  }\DIFaddFL{~
    ...
    \caption{\DIFaddFL{Attitude stabilization without adaptive update law}}
    \label{fig:con} 
\end{figure}
\begin{multicols}{2}
\DIFaddend \section{Numerical Examples}\DIFaddbegin \label{sec:numerical_simulation}

\DIFaddend We demonstrate the performance of the proposed control system via numerical simulation.
...
\end{gather*}
\DIFaddbegin \DIFadd{The diagonal matrix \( G \) serves as a weighting matrix for the relative difference between \( R \) and \( R_d \). 
Using this term, the control designer can modify the shape of the attractive error function, given in~}\cref{eqn:A}\DIFadd{, and the resulting behavior of the closed loop system.
Similarly, the constant \( \alpha \) is used to modify the shape of the repulsive error function, given in~}\cref{eqn:B}\DIFadd{.
...
Similarly, in an environment with several densely spaced obstacles, such as that presented in~}\cref{fig:cad_adapt}\DIFadd{, a much larger \( \alpha \) would enable more aggressive manuevers which pass closer to the constraint boundary without violation.
...
It is straightforward to chose \( k_R, k_\Omega, k_\Delta\), using a linear analysis, to satisfy desired response criteria, such as settling time or percent overshoot~\mbox{%DIFAUXCMD
\cite{nise2004}}%DIFAUXCMD
.
}\DIFaddend A body fixed sensor is defined as \(r = [1,0,0]\), while multiple inequality constraints are defined in~\Cref{tab:constraints}.
...
%DIFDELCMD <     \centering 
%DIFDELCMD <     \subfigure[Attitude error vector \(e_R\) \label{fig:eR_con}]{\includegraphics[width=0.5\columnwidth]{eR_con}  }%%%
\DIFdelFL{~
    }%DIFDELCMD < \subfigure[Configuration error \( \Psi \) \label{fig:Psi_con}]{\includegraphics[width=0.5\columnwidth]{figures/Psi_con.pdf} }
%DIFDELCMD <     \subfigure[Angle to constraints \label{fig:con_angles_con}]{\includegraphics[width=0.5\columnwidth]{figures/constraint_angles_con.pdf} }  
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Attitude stabilization without adaptive update law}}
    %DIFAUXCMD
%DIFDELCMD < \label{fig:con} 
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend We show simulation results for the system stabilizing about the desired attitude with and without the adaptive update law from~\Cref{prop:adaptive_control}.
...
\DIFdelbegin %DIFDELCMD < \begin{figure}[H] 
%DIFDELCMD < 	\centering 
%DIFDELCMD <     \subfigure[Configuration error \( \Psi \)\label{fig:Psi_adapt}]{\includegraphics[width=0.5\columnwidth]{Psi_adapt} }%%%
\DIFdelFL{~
    }%DIFDELCMD < \subfigure[Angle to constraints \label{fig:con_angles}]{\includegraphics[width=0.5\columnwidth]{constraint_angles} }\\
%DIFDELCMD <     \subfigure[Disturbance estimate \( \bar \Delta \)  \label{fig:delta_adapt} ]{\includegraphics[width=0.5\columnwidth]{Delta_adapt}}%%%
\DIFdelFL{~
    }%DIFDELCMD < \subfigure[Attitude trajectory \label{fig:cad_adapt}]{\includegraphics[trim=9cm 2.5cm 0 0,clip,width=0.645\columnwidth]{cad_adapt} }
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Attitude stabilization with adaptive update law}}
	%DIFAUXCMD
%DIFDELCMD < \label{fig:adapt} 
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend 

Simulation results without the adaptive update law are shown in~\cref{fig:con}.
\DIFaddbegin \Cref{fig:eR_con} \DIFadd{shows each component of the attitude error vector,~}\cref{eqn:eR}\DIFadd{, over the simulation time span.
}\Cref{fig:Psi_con} \DIFadd{shows the  magnitude of the combined error function,~}\cref{eqn:psi}\DIFadd{.
}\DIFaddend Without the update law, the system does not achieve zero steady state error. 
...
The angle to each of the constraints, \DIFaddbegin \DIFadd{which is measured in degrees and given by }\DIFaddend \( \arccos(r^T R^T v_i) \), is always greater than the specified angle, \( \theta_i \), in~\DIFdelbegin %DIFDELCMD < \cref{tab:constraints}%%%
\DIFdelend \DIFaddbegin \Cref{tab:constraints}\DIFaddend .

\Cref{fig:adapt} shows the results with the addition of the adaptive update law.
\DIFaddbegin \Cref{fig:Psi_adapt,fig:con_angles} \DIFadd{are equivalent to~}\cref{fig:Psi_con,fig:con_angles_con} \DIFadd{with the exception of the addition of the adaptive update law.
}\DIFaddend The addition of the adaptive update law allows the system to converge to the desired attitude in the presence of constraints.
The path of the body fixed sensor in the inertial frame, namely \( R r \), is illustrated in~\cref{fig:cad_adapt} \DIFaddbegin \DIFadd{by the blue trajectory.
The rendering of the spacecraft is presented in the desired, or final, orientation of the simulation}\DIFaddend .
The \DIFdelbegin \DIFdel{initial attitude is represented with the green circle while the finalattitude is marked with a green \(\times\).
The }\DIFdelend inequality constraints from~\Cref{tab:constraints} are depicted as red cones, where the cone half angle is \( \theta \).
The control system is able to asymptotically converge to zero attitude error.
\Cref{fig:con_angles} shows that the angle\DIFaddbegin \DIFadd{, }\DIFaddend \( \arccos(r^T R^T v_i) \) \DIFaddbegin \DIFadd{and measured in degrees, }\DIFaddend between the body fixed sensor and each constraint is satisfied for the entire maneuver.
...
\DIFdelbegin \section{\DIFdel{Experiment on Hexrotor UAV}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Attitude Parameterizations}}\label{ssec:attitude_parameterization}
\end{multicols}
\begin{figure} 
  \centering 
    \subfigure[Configuration error \( \Psi \)\label{fig:Psi_adapt}]{\includegraphics[width=0.4\columnwidth]{Psi_adapt} }\DIFaddFL{~
    }\subfigure[Angle to each constraint \label{fig:con_angles}]{\includegraphics[width=0.4\columnwidth]{constraint_angles} }\\
    \subfigure[Disturbance estimate \( \bar \Delta \) components \label{fig:delta_adapt} ]{\includegraphics[width=0.4\columnwidth]{Delta_adapt}}\DIFaddFL{~
  ...
  \caption{\DIFaddFL{Attitude stabilization with adaptive update law}}
  \label{fig:adapt} 
\end{figure}
\begin{multicols}{2}
%DIF >  Discuss the benefits of using this geometric representation as compared to Euler angles or quaternions
\DIFadd{Attitude parameterizations, such as Euler angles and Quaternions, are frequently used in the aerospace and astrodynamics communities~\mbox{%DIFAUXCMD
\cite{vallado2007}}%DIFAUXCMD
.
For example, Euler angle sequences are frequently used to describe the transformation between a variety of reference frames used to describe the position and orientation of the orbit of Earth satellites~\mbox{%DIFAUXCMD
\cite{vallado2007}}%DIFAUXCMD
.
In addition, quaternions were used during the operation of Skylab and the NASA Space Shuttle~\mbox{%DIFAUXCMD
\cite{hughes2004}}%DIFAUXCMD
.
...
%DIF >  singularities and the problems involved with using Euler angles
\DIFaddbegin \DIFadd{Euler angle sequences are a minimum, three-parameter set of angles which describe the transformation between two reference frames.
Using Euler angles, we can represent any general rotation as a sequence of three intermediate rotations~\mbox{%DIFAUXCMD
\cite{shuster1993}}%DIFAUXCMD
.
By convention, there are }\num{24} \DIFadd{possible Euler angle sequences for any given rotation.
...
Furthermore, it has been shown that no minimal attitude representation can describe orientations both globally and without singularities~\mbox{%DIFAUXCMD
\cite{hughes2004,bhat2000}}%DIFAUXCMD
.
These singularities can cause significant difficulties during control design and hardware implementation.
}

\DIFadd{To demonstrate the effect of the kinematic singularities inherent with Euler angles we will represent the attitude of the body fixed reference frame, \( \vecbf{b}_i \), with respect to the inertial frame, \( \vecbf{e}_i\), in terms of the \( 3-1-3\) Euler angle sequence.
...
}\begin{align}\DIFadd{\label{eq:euler313}
    \begin{bmatrix}
...
\DIFadd{where \( s_i, c_i \) represent \( \sin \theta_i, \cos \theta_i \) for \( i = \braces{1,2,3}\).
Using this representation, the kinematic differential equations for the associated Euler angles are given as
}\begin{align}\DIFadd{\label{eq:euler313_diff}
    \begin{bmatrix}
...
\DIFadd{From~}\cref{eq:euler313_diff}\DIFadd{, it is immediately clear that a singularity exists when \( \sin \theta_2 = 0 \) or equivalently, \( \theta_2 = 0, \pm \pi \). 
...
As a result, Euler angles are not appropriate for systems which experience large angular rotations, such as those demonstrated in~}\cref{fig:adapt}\DIFadd{, or control systems which rely on the angular velocities \( \theta_i \).
%DIF >  discuss the unwinding phenomenon with quaternions
}

\end{multicols}
\begin{figure}[h]
    \centering 
    \subfigure[{Configuration error \( \Psi \)} \label{fig:Psi_tv} ]{\includegraphics[width=0.3\columnwidth]{Psi_tv} }\DIFaddFL{~
    }\subfigure[{Angle to constraint } \label{fig:con_angle_tv}]{\includegraphics[width=0.3\columnwidth]{figures/constraint_angle_tv.pdf} }\DIFaddFL{~
    ...
    \caption{\DIFaddFL{Time-varying external disturbance simulation}}
    \label{fig:tv} 
\end{figure}
\begin{multicols}{2}
\subsection{\DIFadd{Time-varying Disturbance}}\label{ssec:time_varying}

\DIFadd{The form of the uncertainty, given in~}\cref{eqn:Wdot}\DIFadd{, is commonly used in the adaptive control literature~\mbox{%DIFAUXCMD
\cite{LeeITCST13,ioannou2012}}%DIFAUXCMD
. 
...
For example, Euler's law gives the relationship for the rate of change of angular momentum as
    ...
    \DIFadd{M_{ext} = \dot{\vecbf{H}} = \dot{J} \vecbf{\Omega} + J \dot{\vecbf{\Omega}} .
}\end{align*}
\DIFadd{Using this, we can see that an instantaneous change in \( J \) is proportional to an external moment.
Finally, it has been shown that this adaptive control formulation is able to handle time-varying disturbances under some mild assumptions~\mbox{%DIFAUXCMD
\cite{ioannou2012}}%DIFAUXCMD
. 
}

\DIFadd{We demonstrate the ability to handle an uncertain time-varying disturbance via numerical example.
The system is identical to the one presented in~}\cref{sec:numerical_simulation}\DIFadd{, however we modify the external disturbance. 
The external disturbance is the superposition of constant and time-varying terms as
}\begin{align*}
    \DIFadd{\Delta = \begin{bmatrix} 0.2 \\ 0.2 \\0.2 \end{bmatrix} + 0.02 \begin{bmatrix} \sin 9 t \\ \cos 9 t \\ \frac{1}{2} \parenth{\sin 9t + \cos 9t}\end{bmatrix} \si{\newton\meter}.
}\end{align*}
\DIFadd{We define a constraint in the inertial frame as \( v = [\frac{1}{\sqrt{2}}, \frac{1}{\sqrt{2}}, 0]^T \) with \( \theta = \ang{12} \).
...
\Cref{fig:tv} \DIFadd{demonstrates the ability for the adaptive controller, which is presented in~}\Cref{prop:adaptive_control}\DIFadd{, to handle time-varying disturbances.
}\Cref{fig:Psi_tv} \DIFadd{shows the non-dimensional value of the configuration error function and demonstrates that the adaptive controller is able to stabilize the system to the desired attitude configuration.
In addition,~}\cref{fig:con_angle_tv} \DIFadd{shows that the constraint is never violated as the angle between the body-fixed sensor \( r \) and the constraint \( v \) is greater than }\SI{12}{\degree} \DIFadd{over the entire attitude maneuver.
We can see in~}\cref{fig:Delta_tv} \DIFadd{that that estimate \(\bar \Delta \) for each of the components accurately tracks the true disturbance after approximately~}\SI{5}{\second}\DIFadd{.
}

\section{\DIFadd{Experiment on Hexrotor UAV}}\label{sec:experiment}
\begin{figure}[H]
    \centering
    \includegraphics[width = 0.75\columnwidth]{hexrotor}
    \caption{\DIFaddFL{Attitude control testbed~}\label{fig:hexrotor}}
\end{figure}
\DIFaddend A hexrotor unmanned aerial vehicle (UAV), as seen in~\cref{fig:hexrotor}, has been developed at the Flight Dynamics and Controls Laboratory (FDCL) at the George Washington University~\cite{kaufman2014}.
...
\DIFdelbegin %DIFDELCMD < \begin{figure}[H]
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics[width = 0.75\columnwidth]{hexrotor}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Attitude control testbed~}%DIFDELCMD < \label{fig:hexrotor}%%%
}
%DIFAUXCMD
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend Attitude information is measured by \DIFdelbegin \DIFdel{an onboard IMU and a }\DIFdelend \DIFaddbegin \DIFadd{a combination of both on and off board sensor systems.
...
A }\DIFaddend Vicon motion capture system \DIFdelbegin \DIFdel{in the test area. 
}\DIFdelend \DIFaddbegin \DIFadd{is installed within the test environment and used to provide high accuracy attitude measurements. 
...
}\DIFaddend The control input is computed \DIFdelbegin \DIFdel{onboard the hexrotor using data transmitted via WiFi in a client/server model}\DIFdelend \DIFaddbegin \DIFadd{on-board, using the full state measurement, and implemented at approximately }\SI{100}{\hertz}\DIFaddend .
...
The attitude control system is identical to the one presented in~\Cref{prop:adaptive_control} with the exception of a gravity moment term\DIFdelbegin \DIFdel{and the following parameters}\DIFdelend \DIFaddbegin \DIFadd{, \( M_g = r_{cg} \times m g R^T e_3\) which represents the gravitational moment due to the difference between the center of mass and the center of rotation. 
In addition, the following parameters were also modified}\DIFaddend : \(k_R = 0.4, k_\Omega = 0.7 ,c = 0.1 , \alpha = 8 \text{ and } k_\Delta = 0.05\) \DIFdelbegin \DIFdel{.
}%DIFDELCMD < \begin{figure}[H]
%DIFDELCMD < 	\centering 
%DIFDELCMD <     \subfigure[{Attitude error vector \(e_R\)} \label{fig:eR_exp}]{\includegraphics[width=0.5\columnwidth]{eR_exp}  }%%%
\DIFdelFL{~
	}%DIFDELCMD < \subfigure[{Configuration error \( \Psi \)} \label{fig:Psi_exp} ]{\includegraphics[width=0.5\columnwidth]{Psi_exp} }\\
%DIFDELCMD < 	\subfigure[{Control input \( u\) }\label{fig:u_exp}]{\includegraphics[width=0.5\columnwidth]{u_exp}  }%%%
\DIFdelFL{~
	}%DIFDELCMD < \subfigure[{Attitude Trajectory } \label{fig:traj_exp}]{\includegraphics[width=0.5\columnwidth]{traj_exp} }\\
%DIFDELCMD <     \subfigure[{Angle to constraint } \label{fig:con_angle_exp}]{\includegraphics[width=0.5\columnwidth]{figures/constraint_angles_exp.pdf} }
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Constrained Attitude stabilization experiment}}
	%DIFAUXCMD
%DIFDELCMD < \label{fig:exp} 
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{to account for the differences in the hardware model of the hexrotor.
}

\DIFaddend The experimental results are shown in~\Cref{fig:exp}.
\DIFaddbegin \Cref{fig:eR_exp} \DIFadd{shows the behavior of each of the components of the attitude error vector, defined by~}\cref{eqn:eR}\DIFadd{, over the experiment time span.
}\Cref{fig:Psi_exp} \DIFadd{shows the time history of the attitude error function, defined by~}\cref{eqn:psi}\DIFadd{.
}\Cref{fig:u_exp} \DIFadd{shows the magnitude of each component of the control input in }\si{\newton\meter}\DIFadd{, which is computed from~}\cref{eqn:adaptive_control}\DIFadd{.
Finally,~}\cref{fig:con_angle_exp} \DIFadd{shows the angle between the body-fixed sensor and the obstacle in degrees.
}\DIFaddend In order to maneuver the system ``close" to the constrained zone we utilize several intermediary set points on either side of the obstacle.
...
\begin{figure}[H]
    \centering 
    \subfigure[{Attitude error vector \(e_R\)} components\label{fig:eR_exp}]{\includegraphics[width=0.3\columnwidth]{eR_exp}  }\DIFaddFL{~
    }\subfigure[{Configuration error \( \Psi \)} \label{fig:Psi_exp} ]{\includegraphics[width=0.3\columnwidth]{Psi_exp} }\DIFaddFL{~
    }\subfigure[{Control input \( u\) } components\label{fig:u_exp}]{\includegraphics[width=0.3\columnwidth]{u_exp}  }\\
    \subfigure[{Attitude Trajectory } \label{fig:traj_exp}]{\includegraphics[width=0.3\columnwidth]{traj_exp} }\DIFaddFL{~
    ...
    \caption{\DIFaddFL{Constrained Attitude stabilization experiment}}
    \label{fig:exp} 
\end{figure}
\begin{multicols}{2}
...
The \DIFdelbegin \DIFdel{variation of}\DIFdelend \DIFaddbegin \DIFadd{infinitesimal variation of a rotation matrix is defined as
}\begin{align*}
    \DIFadd{\delta R = \left. \diff{}{\epsilon} \right|_{\epsilon=0} R \exp{\epsilon \hat{\eta}} = R \hat{\eta} .
}\end{align*}
\DIFadd{Using this, the variation of}\DIFaddend ~\cref{eqn:A} is taken with respect to \DIFdelbegin \DIFdel{\( \delta R = R \hat \eta \) }\DIFdelend \DIFaddbegin \DIFadd{\( R \) }\DIFaddend as
\begin{align*}
	\dirDiff{A}{R} &= \eta \cdot \frac{1}{2} \left( G R_d^T R - R^T R_d G\right)^\vee ,
%	&= -\frac{1}{2} \tr{G R_d^T \left( R \hat{\eta}\right)} \\
\end{align*}
where we used~\cref{eqn:hat1} \DIFaddbegin \DIFadd{to achieve the simplified form}\DIFaddend .
...
%DIFDELCMD < %%%
\DIFdel{The desired equilibrium occurs at \( R=R_d\) and results in \( e_R = 0 \) and \( \Psi = A = 0\). }
\DIFadd{We show~\cref{item:prop_psi_quadratic} by computing the hessian of \( \Psi \), namely \( \Hess(\Psi) \),  using the chain rule as }
\begin{align*}
    &\DIFadd{\Hess (\Psi)\cdot} \DIFadd{ (\delta R,\delta R)= (\mathbf{D}_R\parenth{\mathbf{D}_R A \cdot \delta R}\cdot \delta R) B}\\
    & \DIFadd{+ \parenth{\dirDiff{A}{R} }\parenth{\dirDiff{B}{R}} + \parenth{\dirDiff{B}{R} }\parenth{\dirDiff{A}{R}}}\\ 
    & \DIFadd{+(\mathbf{D}_R\parenth{\mathbf{D}_R B \cdot \delta R}\cdot \delta R) A.}
\end{align*}
\DIFadd{The first order derivative of \( A(R) \) and \( B(R) \) are given by~}\cref{eqn:eRA,eqn:eRB}\DIFadd{. 
The hessian of \( A(R) \) is computed as 
}\begin{align*}
    \DIFadd{\dirDiff{\parenth{\dirDiff{A}{R}}}{R} }&\DIFadd{= \eta \cdot \frac{1}{2}\parenth{G R_d^T \delta R - \delta R^T R_d G}^\vee , }\\
    &\DIFadd{= \eta \cdot \frac{1}{2} \parenth{G R_d^T R \hat \eta + \hat \eta R^T R_d G}^\vee,}\\
    &\DIFadd{= \eta \cdot \frac{1}{2} \bracket{\parenth{\braces{\tr{R^T R_d G} I - R^T R_d G} \eta}^\wedge}^\vee , }\\
    &\DIFadd{= \eta \cdot \frac{1}{2} \parenth{\tr{R^T R_d G} I - R^T R_d G}\eta ,
}\end{align*}
\DIFadd{where we used the scalar triple product rule, from~}\cref{eqn:STP}\DIFadd{, to arrive at the final form.
The hessian of \( B(R) \) is computed as
}\begin{align*}
    \DIFadd{\dirDiff{\parenth{\dirDiff{B}{R}}}{R} =}& \DIFadd{\eta \cdot \left[\frac{\parenth{\delta R^T v}^\wedge r}{\alpha\parenth{r^T R^T v - \cos \theta}} \right. }\\
    & \DIFadd{\left. - \frac{\parenth{R^T v}^\wedge r \parenth{r^T \delta R^T v}}{\alpha \parenth{r^TR^Tv - \cos \theta}^2}   \right] .
}\end{align*}
\DIFadd{The term \( \parenth{\delta R^T v}^\wedge r\) is simplified as
}\begin{align*}
    \DIFadd{\parenth{\delta R^T v}^\wedge r }&\DIFadd{= - \hat{r} \delta R ^T v , }\\
    &\DIFadd{= - \hat{r} \parenth{R \hat \eta}^T v , }\\
    &\DIFadd{= \hat{r} \parenth{\hat{\eta} R^T } v , }\\
    &\DIFadd{= - \hat{r} \parenth{R^T v}^\wedge \eta ,
}\end{align*}
\DIFadd{where we utilized the hat map property from~}\cref{eqn:cross_product}\DIFadd{.
Similarly, the term \( r^T \delta R^T v \) is simplified as
}\begin{align*}
    \DIFadd{r^T \delta R^T v }&\DIFadd{= r^T \parenth{-\hat \eta R^T}v , }\\
    &\DIFadd{= r^T \parenth{R^T v}^\wedge \eta .
}\end{align*}
\DIFadd{The hessian of \( B(R) \) then becomes
}\begin{align*}
    \DIFadd{\dirDiff{\parenth{\dirDiff{B}{R}}}{R} =}& \DIFadd{\eta \cdot \left[ - \frac{ \hat r \parenth{R^T v}^\wedge}{\alpha \parenth{r^T R^T v - \cos \theta}} \right. }\\ 
    & \DIFadd{\left. - \frac{\parenth{R^T v}^\wedge r r^T \parenth{R^T v}^\wedge }{ \alpha\parenth{r^T R^T v - \cos \theta}^2}  \right] \eta .
}\end{align*}
\DIFadd{Using these terms, we evaluate \( \Hess{\Psi} \) at the desired attitude \( R = R_d \) as follows. Since $A=0$ and $\mathbf{D}_R A=0$ at $R=R_d$, 
}\begin{align*}
    \DIFadd{\Hess(\Psi)\cdot (\delta R,\delta R)\big|_{R=R_d} = \eta \cdot \frac{1}{2} B \parenth{\tr{G}I -  G} \eta , 
}\end{align*}
\DIFadd{which is positive definite since \( B > 1\) and \( \sum g_i > g_i\). 
...
Therefore, in \( D \) the configuration error function is quadratic and the bounds in~}\cref{item:prop_psi_quadratic} \DIFadd{are valid according to~\mbox{%DIFAUXCMD
\cite[Proposition 6.30]{bullo2004}}%DIFAUXCMD
.
}

\DIFaddend The proof of \cref{item:prop_era_upbound} is available in~\cite{LeeITCST13}.
...
\DIFdelbegin \DIFdel{The }\DIFdelend %DIF > We choose the domain \( D \) to exclude the undesired critical points of \( A(R) \) as well as the infeasible regions defined by the state inequality constraints. 
%DIF > Furthermore,~\cref{proof:config_error} shows that \(D \) is an open neighborhood of the desired attitude configuration, \( R_d\).
%DIF > The proof of the upper bound of \( A(R) \) is given in~\cite{LeeITCST13}.
\DIFaddbegin 


\DIFadd{Consider the open neighborhood $D$ of $R=R_d$ defined in~}\Cref{prop:config_error}\DIFadd{.
The proof of the upper bound of \( A(R) \) is given in~\mbox{%DIFAUXCMD
\cite{LeeITCST13}}%DIFAUXCMD
.
The }\DIFaddend selected domain ensures that the configuration error function is bounded \( \Psi < \psi \).
...
Next, we show~\DIFdelbegin \DIFdel{~}\DIFdelend \cref{eqn:E_bound,eqn:F_bound} using the Frobenius norm.
...
\begin{align*}
	\mathcal{V} = \frac{1}{2} e_\Omega \cdot J e_\Omega + k_R \Psi + c J e_\Omega \cdot e_R + \frac{1}{2 k_\Delta} e_\Delta \cdot e_\Delta , \DIFdelbegin %DIFDELCMD < \label{eqn:v_adapt}
%DIFDELCMD < %%%
\DIFdelend %DIF > \label{eqn:v_adapt}
\end{align*}
over the domain \( D \)\DIFdelbegin \DIFdel{in~}%DIFDELCMD < \cref{eqn:domain}%%%
\DIFdel{. 
From~}%DIFDELCMD < \Cref{prop:eR_dot_bound}%%%
\DIFdel{, the Lyapunov function is }\DIFdelend \DIFaddbegin \DIFadd{, defined in~}\cref{prop:config_error}\DIFadd{. 
In this set, the properties of~}\Cref{prop:config_error,prop:error_dyn} \DIFadd{are satisfied.
%DIF > Furthermore, the set \( D \) is an open neighborhood of the desired attitude and Lyapunov stability results are valid in this set.
}

\DIFadd{From~}\Cref{prop:config_error}\DIFadd{, the configuration error function is locally quadratic and it is }\DIFaddend bounded in \( D \) by\DIFaddbegin \DIFadd{~}\cref{eq:psi_bound}\DIFadd{.
Using this, the Lyapunov function \( \mathcal{V} \) is bounded by
}\DIFaddend \begin{align*} %\label{eqn:v_upper_bound}
	\DIFaddbegin \DIFadd{z^T W_1 z \leq }\DIFaddend \mathcal{V} \leq z^T W\DIFaddbegin \DIFadd{_2 }\DIFaddend z ,
\end{align*}
where \( e_\Delta = \Delta - \bar{\Delta} \), \( z = [\|e_R\|,\|e_\Omega\|,\|e_\Delta\|]^T\in\R^3 \) and the \DIFdelbegin \DIFdel{matrix \(W \in \R^{3 \times 3}\) is given by
}\begin{eqnarray*}
	\DIFdel{W = \begin{bmatrix}
...
%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{matrices \(W_1,W_2 \in \R^{3 \times 3}\) are given by
}\begin{align*}
	\DIFadd{W_1 }& \DIFadd{= \begin{bmatrix}
		k_R n_1 & -\frac{1}{2} c \lambda_M & 0 \\
		-\frac{1}{2} c \lambda_M & \frac{1}{2} \lambda_m & 0 \\
		0 & 0 & \frac{1}{2 k_\Delta}
	\end{bmatrix},}\\
	\DIFadd{W_2 }& \DIFadd{= \begin{bmatrix}
...
%DIF > \begin{gather*}
%DIF > \frac{1}{2}\lambda_m k_R n_1 - \frac{1}{4}c^2 \lambda_M^2 >0\\
%DIF > \frac{2\lambda_m k_R n_1}{\lambda_M^2} > c^2 \\
%DIF > \end{gather*}
\DIFaddend The time derivative of \( \mathcal{V}\) with the control \DIFdelbegin \DIFdel{inputs}\DIFdelend \DIFaddbegin \DIFadd{input defined by}\DIFaddend ~\cref{eqn:adaptive_control} is given \DIFdelbegin \DIFdel{by
...
\end{align*}
Using~\Cref{prop:eR_dot_bound}\DIFaddbegin \DIFadd{, }\DIFaddend an upper bound on \( \dot{\mathcal{V}} \) is written as
\begin{gather*}
...
\end{gather*}
%DIF > \begin{gather*}
%DIF > k_R c (k_\Omega -c \lambda_M H) -\frac{1}{4}k_\Omega^2 c^2 < 0\\
%DIF > k_R (k_\Omega -c \lambda_M H) -\frac{1}{4}k_\Omega^2 c < 0\\
%DIF > 4k_R k_\Omega -c(4k_R \lambda_M H +k_\Omega^2) <0
%DIF > \end{gather*}
\DIFaddbegin 

\DIFaddend If \( c \) is chosen such that~\cref{eqn:c_bound} is satisfied \DIFdelbegin \DIFdel{the matrix }\DIFdelend \DIFaddbegin \DIFadd{then the matrices \( W_1, W_2 \) and }\DIFaddend \( M \) \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend positive definite.
This implies that \DIFaddbegin \DIFadd{$\mathcal{V}$ is positive definite and decrescent, and }\DIFaddend $\dot{\mathcal{V}}$ is negative semidefinite \DIFdelbegin \DIFdel{and $\lim_{t\to\infty} \zeta=0$. As the Lyapunov function is non-increasing $z$ is uniformly bounded. }\DIFdelend \DIFaddbegin \DIFadd{in the domain $D$. As such, the zero equilibrium is stable in the sense of Lyapunov, and all of the error variables are bounded. Furthermore, $\lim_{t\to\infty} \zeta=0$ according to the LaShalle-Yoshizawa theorem~\mbox{%DIFAUXCMD
\cite{khalil1996}}%DIFAUXCMD
. 
}

%DIF > and 
%DIF > As the Lyapunov function is non-increasing $z$ is uniformly bounded. 
\DIFaddend %	
%	
...
%DIF < \addtolength{\textheight}{-12cm}   % This command serves to balance the column lengths
                                  %DIF <  on the last page of the document manually. It shortens
                                  %DIF <  the textheight of the last page by a suitable amount.
                                  %DIF <  This command does not take effect until the next page
                                  %DIF <  so it should come on the page before the last. Make
                                  %DIF <  sure that you do not shorten the textheight too much.
                                  \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \bibliography{BibMaster,library}
\bibliographystyle{IEEEtran}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \biography{profile}{Shankar Kulumani}
...
%DIFDELCMD < %%%
\DIFdelend \biography{Lee}{Taeyoung Lee}

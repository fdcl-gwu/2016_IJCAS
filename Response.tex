...
\begin{enumerate}
\item \textit{Please give a precise definition of variation used in the paper, e.g.,~\cref{eq:dirDiff_A,eq:dirDiff_B}}
...
\item \textit{This is serious. In~\cref{sec:attitude_control}, the upper bounds listed in~\cref{prop:eR_dot_bound} are given. Well, these should be the properties that your controller guarantees not come from the set D you've defined. Since these properties are used to derive the bound H which is used to derive the result in~\cref{prop:adaptive_control}, the stability proof under the proposed controller is far from complete.}
...
%Here we present the modifications and provide additional detail. 
%In addition, we duplicate the modifications from the manuscript in this section as well.
%
...
%        \end{align}
%\end{correction}
%and proven in~\Cref{proof:config_error}, as shown here:
%\begin{correction}
...
%of the metric space \( \parenth{R \in \SO, \Psi(R,R_d)} \) is an open neighborhood of \( R_d \) if there exists an \( \epsilon > 0 \) such that for any \( R \in \SO | \Psi(R,R_d) < \epsilon \), it is also true that \( R \in D \).
...
%\end{correction}
%
%The proof of~\Cref{prop:adaptive_control} given in~\Cref{proof:adaptive_control} has been improved to explicitly demonstrate that \( \mathcal{V} \) is positive definite.
%\begin{correction}
...
%    \end{align*}
%    over the domain \( D_3 = D_1 \cap D_2\), which ensures that the properties of~\Cref{prop:config_error,prop:error_dyn} are satisfied.
%
%    From~\Cref{prop:config_error}, the configuration error function is locally quadratic and it is bounded in \( D_3 \) by~\cref{eq:psi_bound}~\cite[Definition 6.28]{bullo2004}.
...
%    \begin{gather}
%        0 < c < \min \braces{\sqrt{\frac{2 k_R n_1}{\lambda_m}},\sqrt{\frac{2 k_R n_2}{\lambda_M}}, \frac{4 k_R k_\Omega}{k_\Omega^2 + 4 k_R \lambda_M H}} .
%    \end{gather}
...
\item \textit{Abuse of notation 'hat' and 'vee' in~\crefrange{eqn:hatxy}{eqn:xAAx}}
...
\item \textit{The units of y axis of~\Cref{fig:config_error,fig:con,fig:adapt,fig:tv,fig:exp} should be mentioned in the manuscript.}
...
In addition, the captions of~\Cref{fig:config_error,fig:adapt,fig:con,fig:tv,fig:exp} have been modified to mention the fact that they are plotting the components of a vector in a single plot or using a subplot.
\end{enumerate}

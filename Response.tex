...
The revised manuscript, through~\cref{prop:config_error,prop:adaptive_control}, demonstrates several properties of the error function and the error variables which are shown in the open neighborhood $D$ of the desired attitude, defined in~\cref{eq:psi_quadratic_domain}.  Later in the stability proof of~\cref{prop:adaptive_control}, it is shown that the chosen Lyapunov function is positive definite in the same domain $D$, and its time-derivative is negative-semidefinite in $D$.  As $D$ is an open neighborhood of the desired attitude, the revised manuscript completes the stability proof. 
